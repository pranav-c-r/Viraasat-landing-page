...
function TajMahalModel({ collisionDetector }) {
  const { scene } = useGLTF('/models/tajmahal_base_basic_shaded.glb');
...
export default function TajMahalExplorer() {
  ...
  const handlePlayerMove = (position) => {
    ...
    const distanceFromTajMahal = position.distanceTo(new Vector3(0, 0, 0));
    if (distanceFromTajMahal < 5) {
  ...
  const handlePhotoMode = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setInfoText('🏛️ Enhanced Taj Mahal Explorer - Explore Taj Mahal');
        setPhotoMode(false);
      }, 2000);
    }, 500);
  };
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden">
      ...
      >
        ...
        {/* Models */}
        <TajMahalModel collisionDetector={collisionDetector} />
      ...
      {showInstructions && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-80">
          <div className="text-center text-white bg-black bg-opacity-90 p-8 max-w-lg mx-auto rounded-lg border border-gold">
            <h2 className="text-2xl font-bold mb-4 text-yellow-400">🏛️ Enhanced Taj Mahal Explorer</h2>
