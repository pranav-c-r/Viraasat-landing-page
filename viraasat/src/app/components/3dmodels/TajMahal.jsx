...
function Forest({ collisionDetector }) {
  const trees = useMemo(() => {
    return [
      { position: [26, 0, -25], size: 1.2 },
    
      { position: [-40, 0, -35], size: 1.3 },
   
      { position: [-20, 0, -30], size: 1.0 },
    ];
  }, []);
  return (
    <group>
...
export default function TajMahalExplorer() {
    ...
    return (
        <div className="w-full h-screen relative">
            ...
            {showInstructions && (
                <div className="absolute top-4 left-0 right-0 overflow-y-scroll text-center text-white bg-black/90 p-6 max-w-lg mx-auto rounded-lg backdrop-blur-sm border border-yellow-500/30">
                    ...
                    <p className="text-green-300 hidden md:block mb-4 text-sm bg-green-500/10 p-2 rounded border border-green-500/20">
