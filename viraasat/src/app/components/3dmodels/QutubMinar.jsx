...
function FirstPersonControls({ speed = 8, collisionDetector, audioManager, onMove }) {
    ...
    useEffect(() => {
        ...
        camera.position.set(0, currentHeight.current, 38);
...
function QutubMinarModel({ collisionDetector }) {
    const { scene } = useGLTF('/models/qutubminar_compressed.glb');
    useEffect(() => {
    ...
    return <primitive object={scene} scale={15.5} position={[0, 0, 0]} />; 
}
      ...
      <div className={`fixed left-0 top-0 h-full w-80 bg-gradient-to-b from-slate-900/95 via-slate-800/95 to-slate-900/95 text-white shadow-2xl transform transition-all duration-300 ease-in-out z-40 backdrop-blur-sm ${
        ...
        <div className="p-6 h-full overflow-y-auto">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-yellow-400 drop-shadow-lg">Qutub Minar Settings</h2>
            <div className="text-sm text-gray-300 text-right">
              <div>Qutub Minar Explorer</div>
...
export default function QutubMinarExplorer() {
    const [infoText, setInfoText] = useState('Click to explore the magnificent Qutub minar from a realistic human perspective');
    ...
    if (position.z < 4 && position.z > -2) {
        setInfoText('You are standing before the magnificent minaret at natural human height');
    } else if (position.x > 20) {
        setInfoText('Walking through the eastern gardens - experience the monument from a realistic perspective');
    } else if (position.z > 8) {
        setInfoText('Exploring the grounds with a ground-level view of the ancient monument');
    } else if (position.z < -5) {
        setInfoText('You are walking behind the ancient monument among the towering trees');
    } else {
        setInfoText('Wandering the sacred monument grounds from a natural human perspective');
    }
}; return (
        <div className="w-full h-screen relative">
            <Canvas shadows camera={{ position: [0, 1.55, 8], fov: 75 }}>
                ...
                <TimeOfDayUpdater 
                    ...
                    timeMode={timeMode}
                />
                <QutubMinarModel collisionDetector={collisionDetector} />  {/* ← CHANGED */}
            ...
            {showInstructions && (
                <div className="absolute top-4 left-0 right-0 text-center text-white bg-black/90 p-6 max-w-lg mx-auto rounded-lg backdrop-blur-sm border border-yellow-500/30 shadow-2xl">
                    <h3 className="text-2xl font-bold mb-3 text-yellow-300"> Qutub Minar Explorer - Fixed Controls</h3>
