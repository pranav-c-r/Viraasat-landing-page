...
function SanchiStupaModel({ collisionDetector }) {
    const { scene } = useGLTF('/models/sanchi_compressed.glb');
    useEffect(() => {
      ...
      <div className={`fixed left-0 top-0 h-full w-80 bg-gradient-to-b from-slate-900/95 via-slate-800/95 to-slate-900/95 text-white shadow-2xl transform transition-all duration-300 ease-in-out z-40 backdrop-blur-sm ${
        ...
        <div className="p-6 h-full overflow-y-auto">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-yellow-400 drop-shadow-lg">Sanchi Stupa Settings</h2>
            <div className="text-sm text-gray-300 text-right">
              <div>Sanchi Stupa Explorer</div>
...
export default function SanchiStupaExplorer() {
    const [infoText, setInfoText] = useState('Click to explore the magnificent Sanchi Stupa from a realistic human perspective');
    ...
    const handlePlayerMove = (position) => {
        if (position.z > 10) {
            setInfoText('You are approaching the ancient gateways of the Great Stupa.');
        } else if (position.z < -10) {
            setInfoText('Walking the sacred path on the backside of the Stupa.');
        } else if (position.x > 5) {
            setInfoText('Exploring the eastern side, near the lion pillars.');
        } else if (position.x < -5) {
            setInfoText('You are on the western path, experiencing a profound sense of tranquility.');
        } else {
            setInfoText('Wandering the sacred grounds of Sanchi Stupa from a natural human perspective.');
        }
    };
    //
    return (
