...
function FirstPersonControls({ speed = 7.5, collisionDetector, audioManager, onMove }) {
...
function RedFortModel({ collisionDetector }) {
    const { scene } = useGLTF('/models/redfort_compressed.glb');
    useEffect(() => {
      ...
      <div className={`fixed left-0 top-0 h-full w-80 bg-gradient-to-b from-slate-900/95 via-slate-800/95 to-slate-900/95 text-white shadow-2xl transform transition-all duration-300 ease-in-out z-40 backdrop-blur-sm ${
        ...
        <div className="p-6 h-full overflow-y-auto">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-yellow-400 drop-shadow-lg">RED FORT Settings</h2>
            <div className="text-sm text-gray-300 text-right">
              <div>RED FORT Explorer</div>
...
// Main component
export default function RedFortExplorer() {
    const [infoText, setInfoText] = useState('Click to explore the magnificent red fort from a realistic human perspective');
    ...
    if (position.z > 15) {
        setInfoText('You are approaching the majestic Lahore Gate, the main entrance of the Red Fort.');
    } else if (position.z < -10) {
        setInfoText('Wandering behind the fort, near the Yamuna riverfront.');
    } else if (position.x > 5 && position.x < 15 && position.z < 10) {
        setInfoText('Exploring the grounds near the Diwan-i-Aam, or Hall of Public Audience.');
    } else if (position.x < -10 && position.z < 10) {
        setInfoText('You are walking past the Rang Mahal, the "Palace of Colours."');
    } else {
        setInfoText('Wandering the historic grounds of the Red Fort from a natural human perspective.');
    }
};return (
        <div className="w-full h-screen relative">
            <Canvas shadows camera={{ position: [0, 1.55, 8], fov: 75 }}>
                ...
                <TimeOfDayUpdater 
                    ...
                    timeMode={timeMode}
                />
                <RedFortModel collisionDetector={collisionDetector} />  {/* ← CHANGED */}
            ...
            {showInstructions && (
                <div className="absolute top-4 left-0 right-0 text-center text-white bg-black/90 p-6 max-w-lg mx-auto rounded-lg backdrop-blur-sm border border-yellow-500/30 shadow-2xl">
                    <h3 className="text-2xl font-bold mb-3 text-yellow-300">Sanchi Stupa Explorer - Fixed Controls</h3>
