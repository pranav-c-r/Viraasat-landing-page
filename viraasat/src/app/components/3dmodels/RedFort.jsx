...
export default function RedFortExplorer() {
  const [infoText, setInfoText] = useState('🏛️ Enhanced Red Fort Explorer - Click to enter immersive mode');
  ...
  const handlePlayerMove = (position) => {
    ...
    const distanceFromRedFort = position.distanceTo(new Vector3(0, 0, 0));
    if (distanceFromRedFort < 5) {
  ...
  const handlePhotoMode = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setInfoText('🏛️ Enhanced Red Fort Explorer - Explore the ancient temple');
        setPhotoMode(false);
      }, 2000);
    }, 500);
  };
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden">
      ...
      >
        ...
        {/* Models */}
        <RedFortModel collisionDetector={collisionDetector} />
      ...
      {showInstructions && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-80">
          <div className="text-center text-white bg-black bg-opacity-90 p-8 max-w-lg mx-auto rounded-lg border border-gold">
            <h2 className="text-2xl font-bold mb-4 text-yellow-400">🏛️ Enhanced Red Fort Explorer</h2>
